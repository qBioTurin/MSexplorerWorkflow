...
baselines_dec = readRDS(file = "Output/SUPERVISED_DEC/Bacteria_Supervised_decontamOLD.rds")
...
# New labels for category
    sample_data(baselines_dec)$category <- factor(sample_data(baselines_dec)$category, 
                                       levels = c("HEALTHY", "MS"), 
                                       labels = c("HD", "MS"))

# Calculate Bray
    wUF.ordu = ordinate(baselines_dec, method="PCoA", distance="bray")
    baselines_dec = phyloseq(otu_table(baselines_dec), tax_table(baselines_dec), sample_data(baselines_dec))


# Plot PCoA
# Category
    beta1=plot_ordination(baselines_dec, wUF.ordu, type="sample_type", color="category") + 
        theme_classic() + 
        stat_ellipse() + 
...
# Calculate Bray
    ...
    beta2=plot_ordination(MS, ordination, type="sample_type", color="gc_treatment") + 
        theme_classic() + 
        stat_ellipse() + 
        ggtitle("PCoA of bray Curtis distance") + 
        ...
        theme(
            ...
            legend.text = element_text(size = 15),
            legend.title = element_text(size = 15))
    saveRDS(beta2,gsub(" ", "", paste(output_folder,kingdom,"_beta_gc.rds")))
}
Beta(baselines_dec,"Bacteria",output_folder)
