...
Alpha <- function(baselines_dec, Domain, output_folder) {
  baselines_dec_richness <- estimate_richness(baselines_dec, split = TRUE, measures = c("Observed", "Shannon", "Simpson"))
  ...
  baselines_dec_metadata1 <- baselines_dec_metadata %>%
    tidyr::gather(Observed, Shannon, Simpson,  key = "index", value = "value")
  ###### healty vs ms
