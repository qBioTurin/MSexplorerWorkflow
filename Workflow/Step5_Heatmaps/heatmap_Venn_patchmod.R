...
baselines_dec = input
  ...
  #mutate(Category=factor(Category,levels=c("HEALTHY","MS","NEG","POS"),labels=c("HV","MS","NEG","POS")))
  ...
  #norm_data_z <- norm_data_z[rownames(norm_data_z) %in% c("Faecalibacterium sp. I4-3-84", "Alistipes sp. dk3624"), ]
...
# Estrazione del dendrogramma delle righe
#dendro_rows <- heatmap$tree_row

# Definizione del numero di cluster da estrarre (ad esempio 3)
#clusters_rows <- cutree(dendro_rows, k = 2)

#clusters_rows <- as.data.frame(clusters_rows)

#merge_cluster and z-scores
#cluster_z <- merge.data.frame(clusters_rows, norm_data_z, by = "row.names")

# Separazione dei dati per cluster
#Bacteroides <- cluster_z[cluster_z$clusters_rows == 1,]
#rownames(Bacteroides) = Bacteroides$Row.names
#Bacteroides <- Bacteroides[, c(-1,-2)]

#Firmicutes <- cluster_z[cluster_z$clusters_rows == 2,]
#rownames(Firmicutes) = Firmicutes$Row.names
#Firmicutes <- Firmicutes[, c(-1,-2)]

#colmean_cluster1 <- colMeans(Bacteroides, na.rm = TRUE)

#colmean_cluster2 <- colMeans(Firmicutes, na.rm = TRUE)

#score <- colmean_cluster2/colmean_cluster1

#write.table(score, file = "(9)Score_Ratio/F_B_ratio_score.txt", sep ="\t", quote = F, dec = ".")


heatmap = ggplotify::as.ggplot(heatmap)

#ggsave(plot = heatmap, filename = filename,
