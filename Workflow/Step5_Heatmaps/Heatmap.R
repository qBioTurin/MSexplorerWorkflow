...
generate.heatmap <- function(Bacteria, Archaea, Eukaryota, filename, filterRows = FALSE, output_folder) {
  ...
  heatmap.kingdom <- function(data, color, paletteLength, col_order = NULL,
    ...
    ann_colors$Phylum <- setNames(
      phylumPalette(n = length(unique(metadata_kingdom$Phylum))),
  ...
  if (!is.null(data_arch[[1]])) {
    heatmap_arch <- heatmap.kingdom(data_arch,
      col_order = colOrder_bact, myColors3, 35,
  ...
  if (!is.null(data_euk[[1]])) {
    ...
    if (!is.null(data_bact[[1]])) {
      heatmap_euk$gtable$widths <- heatmap_bact$gtable$widths
  ...
  ggsave(
    plot = g, filename = gsub(" ", "", paste0(output_folder, filename, ".pdf")),
...
generate.heatmap(
  ...
  Eukaryota = NULL, ,
  filename = paste0("subtentorial_lesions_negative"), output_folder
) ####### LOL
######### gadolinium_contrast############
generate.heatmap(
