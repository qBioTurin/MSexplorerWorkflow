...
#############
# Import Biom
baselines <- import_biom("InputData/MSFinal_json.biom")
...
baselines_metadata <- baselines_metadata %>%
  mutate(
    ...
    across(.cols = c(sex, category, clinical_presentation, gc_treatment,
                     subtentorial_lesions, spinal_cord, gadolinium_contrast,sequencing_batch, lesion_burden,
                     WORSENING, EDSS_DIAGNOSI, EDSS_PROGRESSIONE, Event), 
           .fns = as.factor),
