...
das_limma <- function(baselines_dec, analisys, status, output_folder, Domain, info) {
  ...
  samples <- as.data.frame(as(sample_data(baselines_dec), "data.frame"))

  if (analisys != "category") {
    samples <- samples[!samples$gc_treatment %in% c("healthy", NA), ]
  }
  ...
  el_log<-colnames(log_data)
  el_design<-rownames(design)
  el_miss<-el_log[!el_log %in% el_design]
...
execute_limma <- function() {
  ...
  das_limma(baselines_dec = baselines_decA, analisys = "category", status = "both", output_folder = output_folderMSHD, Domain = "Archaea", info = "")
  ...
  das_limma(baselines_dec = baselines_decA, analisys = "gc_treatment", status = "both", output_folder = output_folderGC, Domain = "Archaea", info = "")
...
r1<-readRDS(file = "Output/SUPERVISED_DEC/Bacteria_Supervised_decontam0.001.rds")
sam_data<-as.data.frame(as(sample_data(r1), "data.frame"))
write.csv(sam_data, file = "InputData/Metadata_MS.csv", row.names = FALSE)
